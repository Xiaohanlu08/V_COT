...
def direct_download(repo_id, filename, revision, repo_type, dst: Path):
    dst.parent.mkdir(parents=True, exist_ok=True)
    if dst.is_file() and dst.stat().st_size > 0:
        print(f"[reuse] {dst} ({dst.stat().st_size} bytes)")
        return

    if shutil.which("curl") is None:
        raise RuntimeError("curl is required but was not found; do not install anything yet.")

    prefix = "datasets/" if repo_type == "dataset" else ""
    encoded_name = "/".join(quote(x) for x in filename.split("/"))

    errors = []
    for base in BASES:
        url = f"{base}/{prefix}{repo_id}/resolve/{revision}/{encoded_name}?download=true"
        tmp = dst.with_suffix(dst.suffix + ".part")
        if tmp.exists():
            tmp.unlink()

        print(f"[download] trying {base}: {filename}", flush=True)
        proc = run([
            "curl",
            "-fL",
            "--retry", "4",
        ...
        if proc.returncode == 0 and tmp.is_file() and tmp.stat().st_size > 0:
            tmp.replace(dst)
            print(f"[ok] {dst} ({dst.stat().st_size} bytes)", flush=True)
            return

        if tmp.exists():
            tmp.unlink()
        errors.append(f"{base}: curl rc={proc.returncode}")

    raise RuntimeError(
        f"All direct GET endpoints failed for {filename}. "
        + " | ".join(errors)
    )
...
def main():
    ...
    print("========== DIRECT-GET METADATA DOWNLOAD ==========")
    ...
    for subset in SUBSETS:
        ...
        direct_download(
            DATASET_REPO, filename, DATASET_REVISION, "dataset", dst
        )
        train_paths[subset] = dst
    ...
    for filename in model_files:
        ...
        direct_download(
            MODEL_REPO, filename, MODEL_REVISION, "model", dst
        )
        model_paths[filename] = dst

    index = json.loads(
        model_paths["stage3/model.safetensors.index.json"].read_text(encoding="utf-8")
    )
    stage3_total_bytes = index.get("metadata", {}).get("total_size")

    grand = Counter()
    assistant_hist_global = Counter()
    user_hist_global = Counter()
    subset_reports = {}
    representative = []
    ...
    for subset in SUBSETS:
        ...
        report = {
            ...
            "assistant_image_count_hist": dict(sorted(ah.items())),
            "user_image_count_hist": dict(sorted(uh.items())),
            "representative_samples": samples,
        }
        subset_reports[subset] = report

        grand.update(stats)
        assistant_hist_global.update(ah)
        user_hist_global.update(uh)
        representative.append({"subset": subset, **samples[0]})

        print(f"\n--- {subset} ---")
        print(json.dumps({
            "rows": report["rows"],
            "rows_with_user_image": report["rows_with_user_image"],
            "rows_with_assistant_image": report["rows_with_assistant_image"],
            "rows_without_assistant_image": report["rows_without_assistant_image"],
            "rows_with_user_and_assistant_images": report["rows_with_user_and_assistant_images"],
            "rows_with_observation_tags": report["rows_with_observation_tags"],
            "assistant_image_count_hist": report["assistant_image_count_hist"],
    ...
    summary = {
        ...
        "download_mode": "direct GET; hf_hub_download intentionally bypassed",
        ...
        "assistant_image_fraction": (
            grand["rows_with_assistant_image"] / total_rows if total_rows else 0.0
        ),
        "observation_tag_fraction": (
            grand["rows_with_observation_tags"] / total_rows if total_rows else 0.0
        ),
        "assistant_image_count_hist": dict(sorted(assistant_hist_global.items())),
        "user_image_count_hist": dict(sorted(user_hist_global.items())),
